 ...
 * Same paper chat UI as student but from mentor perspective.
 */
...
const ChatScreen = () => {
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [...prev, { id: String(Date.now()), sender: 'mentor', text: input.trim(), time: 'Now' }]);
    setInput('');
      ...
      <NotebookBackground>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <View style={styles.header}>
            ...
            <Avatar name="Praveen M." size={40} />
            <View style={styles.headerInfo}>
              <Text style={styles.studentName}>Praveen M.</Text>
              <AvailabilityBadge available={true} label="Student" />
            </View>
            ...
            {messages.map((msg) => {
              ...
              return (
                <View key={msg.id} style={[styles.msgRow, isMine && styles.msgRowRight]}>
                  {!isMine && <Avatar name="Praveen M." size={30} />}
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    shadowColor: Colors.borderBlack, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 1, shadowRadius: 0, elevation: 3,
  },
  ...
  bubble: {
    borderWidth: 2.5, borderColor: Colors.borderBlack, borderRadius: Radius.md, padding: 12,
    shadowColor: Colors.borderBlack, shadowOffset: { width: 2, height: 2 }, shadowOpacity: 1, shadowRadius: 0, elevation: 2,
  },
  bubbleMine: { backgroundColor: Colors.stickyBlue },
  ...
  inputField: {
    flex: 1, backgroundColor: Colors.paperCream, borderWidth: 2, borderColor: Colors.borderInk,
  ...
  sendBtn: {
    width: 40, height: 40, borderWidth: 2.5, borderColor: Colors.borderBlack, borderRadius: Radius.sm,
    alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.inkBlack,
    shadowColor: Colors.borderBlack, shadowOffset: { width: 2, height: 2 }, shadowOpacity: 1, shadowRadius: 0, elevation: 2,
  },
